...
(function(window, factory) {
    ...
    function factory(
        ...
        proto.reloadCells = function() {
            ...
            if (
                jQuery(this.slider).parent().parent().find(".flickity-page-dots")
                .length > 0
            ) {
                var $that = this;
                setTimeout(function() {
        ...
        proto.focus = function() {
            ...
            if (window.pageYOffset != prevScrollY && prevScrollY > 10) {
                // nectar addition - iframe fix
...
(function(window, factory) {
    ...
    proto.dragMove = function(event, pointer, moveVector) {
        ...
        if (!this.options.wrapAround && this.slides.length) {
            // slow drag
            var originBound = Math.max(-this.slides[0].target,
                this.dragStartPosition
            );
            dragX = dragX > originBound ? (dragX + originBound) * 0.5 : dragX;
            var endBound = Math.min(-this.getLastSlide().target,
                this.dragStartPosition
            );
