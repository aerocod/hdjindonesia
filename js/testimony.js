...
(function($) {
    ...
    function NectarTestimonialSlider(
        el,
        type,
        resizeVideoToCover,
    ...
    NectarTestimonialSlider.prototype.createTestimonialControls = function() {
        var $frontEndEditorTestimonialDiv =
            $("body.vc_editor").length > 0 ? "> div" : "blockquote";
        var $that, slide_interval, objectStore;

        if (
            this.type != "multiple_visible" &&
            this.type != "multiple_visible_minimal"
        ) {
            // fadeIn
            ...
            if (this.el.find("blockquote").length > 1) {
                ...
                if (this.el.is('[data-style="minimal"]')) {
                    // add next/prev
                    this.el.append(
                        '<div class="testimonial-next-prev"><a href="#" class="prev fa fa-angle-left"></a><a href="#" class="next fa fa-angle-right"></a></div>'
                    );

                    // bind controls
                    this.el
                        .find(".testimonial-next-prev a")
                        .on("click", this.minimalNextPrevSelect);

                    // start on first
                    if (this.el.find(".active").length == 0) {
                        this.el
                            .find(".slides " + $frontEndEditorTestimonialDiv + ":first-child")
                            .addClass("active")
                    ...
                    if (this.el.attr("data-autorotate").length > 0) {
                        $that = this.el;
                        slide_interval =
                            parseInt(this.el.attr("data-autorotate")) < 100 ?
                            4000 :
                            parseInt(this.el.attr("data-autorotate"));
                        objectStore = this;
                    ...
                    this.el
                        ...
                        .css(
                            "width",
                            this.el.find(".controls ul li").length * 20 + 1 + "px"
                        );
                    this.el
                        .find(".controls")
                        .append(
                            '<span class="out-of">/</span><span class="total">' +
                            this.el.find("blockquote").length +
                            "</span>"
                        );
                ...
                if (!this.el.is('[data-style="minimal"]')) {
                    ...
                    this.el
                        .find(".controls ul li")
                        .on("click", this.defaultPaginationSelect);
                    ...
                    this.el.swipe({
                        swipeLeft: function(e) {
                            $that
                                .find(".controls ul li span.active")
                                .parent()
                                .next("li")
                                .find("span")
                                .trigger("click");
        ...
        if (
            ...
            var $element = $that;
            var $autoplay =
                $that.attr("data-autorotate").length > 1 &&
            ...
            var $testimonialGroupCells =
                this.el.attr("data-style") == "multiple_visible_minimal" ? true : false;
            ...
            if (
                this.flickityEl.find(".vc_element.is-selected > blockquote").length > 0
            ) {
                // starting
                this.flickityEl
                    .find(".vc_element.is-selected > blockquote")
                    .addClass("is-selected");

                // changed
                this.flickityEl.on("select.flickity", function() {
                    objectStore.flickityEl
                        .find(".vc_element > blockquote")
        ...
        $("body").on(
            "click",
            '.testimonial_slider:not([data-style*="multiple_visible"]):not([data-style="minimal"]) .controls li, .testimonial_slider[data-style="minimal"] .testimonial-next-prev a',
            function() {
    ...
    NectarTestimonialSlider.prototype.defaultPaginationSelect = function(
        clicked
    ) {
        ...
        var currentHeight = $target
            .parents(".testimonial_slider")
            .find(".slides blockquote")
            .eq($index)
            .height();
        ...
        $target
            .parents(".testimonial_slider")
            .find(".slides " + $frontEndEditorTestimonialDiv)
            .addClass("no-trans");
        $target
            .parents(".testimonial_slider")
            .find(".slides " + $frontEndEditorTestimonialDiv)
        ...
        $target
            .parents(".testimonial_slider:not(.disable-height-animation)")
            .find(".slides")
            .stop(true, true)
    ...
    NectarTestimonialSlider.prototype.minimalNextPrevSelect = function(clicked) {
        ...
        var $index = $target
            .parents(".testimonial_slider")
            .find(".slides " + $frontEndEditorTestimonialDiv + ".active")
            .index();
        ...
        if ($target.hasClass("next")) {
            if (
                ...
                $target
                .parents(".testimonial_slider")
                .find(".slides " + $frontEndEditorTestimonialDiv).length
            ) {
                $actualIndex = 0;
            } else {
                $actualIndex = $index + 1;
            ...
            if ($index - 1 == -1) {
                $actualIndex =
                    $target
                    .parents(".testimonial_slider")
                    .find(".slides " + $frontEndEditorTestimonialDiv).length - 1;
            } else {
                $actualIndex = $index - 1;
            }
            currentHeight = $target
    ...
    NectarTestimonialSlider.prototype.testimonialRotate = function() {
        ...
        var $currentTestimonial = this.el
            .find(".pagination-switch.active")
            .parent()
            .index();

        // stop the rotation when toggles are closed
        if (
            this.el.parents(".toggle").length > 0 &&
            this.el.parents(".toggle").hasClass("open")
        ) {
            if (!this.el.is('[data-style="minimal"]')) {
    ...
    NectarTestimonialSlider.prototype.testimonialHeightResize = function() {
        if (!this.el.is(".disable-height-animation") &&
            !this.el.is('[data-style*="multiple_visible"]')
        ) {
            var $frontEndEditorTestimonialDiv =
                $("body.vc_editor").length > 0 ? ".slides > div" : ".slides blockquote";
            var $index;

            if (this.el.is('[data-style="minimal"]')) {
                $index = this.el
                    .find($frontEndEditorTestimonialDiv + ".active")
                    .index();
            } else {
                $index = this.el.find(".controls ul li span.active").parent().index();
            }

            var currentHeight = this.el
                .find($frontEndEditorTestimonialDiv)
                .eq($index)
                .height();
            this.el
                .find(".slides")
                .stop(true, true)
                .css({ height: currentHeight + 40 + "px" });
        }
    };

    NectarTestimonialSlider.prototype.testimonialSliderHeight = function() {
        if (
            this.el.is(".disable-height-animation") &&
            !this.el.is('[data-style*="multiple_visible"]')
        ) {
            var $tallestQuote = 0;

            this.el.find("blockquote").each(function() {
                $(this).height() > $tallestQuote ?
                    ($tallestQuote = $(this).height()) :
                    ($tallestQuote = $tallestQuote);
            });
    ...
    NectarTestimonialSlider.prototype.testimonialSliderHeightMinimalMult =
        function() {
            if (this.type == "multiple_visible_minimal") {
                ...
                // set even height
                this.el
                    .find("blockquote > .inner p")
                    .css("height", $tallestQuote + "px");
            }
        };
